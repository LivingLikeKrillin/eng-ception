...
**Listening alignment:** `docs/superpowers/specs/2026-05-01-listening-incrementality-design.md` §12 amendment — `Pattern5HId` is shared. v9 owns the type definition; listening will import from `types/v9.ts` once its own implementation lands.
...
// === 5형식 taxonomy (shared with listening) ===
...
- All platform-agnostic TS layers (types/v9, validate, prompts, mocks, store) are ready for RN port and for listening spec's `Pattern5HId` import
...
2. **organic-first scheduling** — organic sessions are first-class learning events and never touch due-card schedule state; no bypass/postpone (preserves FSRS fidelity). Instead a `bypassedCount` counter escalates a chronically-avoided card into the **listening priority queue**, completing the cross-mode loop (listening↔speaking) bidirectionally (listening spec §12).
3. **Avoidance-threshold N** — cold-start `N=3` static config (N=1–2 = noise; N≥3 = structural avoidance → listening escalation). Roadmap: evolve N into `Pattern Difficulty Weight × Session Density Index` once telemetry accumulates (e.g. `causative-result` tighter at N=2, `want-toV` looser at N=4).
...
7. **Pattern5HChip prominence** — kept **hero/prominent** in v9 (decision: aligns with listening spec A3, "repetition is the lesson"). The expert's 거부감 (school-grammar-label fatigue) concern is logged for a post-launch revisit — possibly a subtle/toggle treatment — pending user-testing data.
