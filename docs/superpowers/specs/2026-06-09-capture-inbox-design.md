...
records/patterns와 동일하게 captures도 비파괴 union: 로컬 captures를 cloud에 업로드(id 기준 idempotent) → 전부 성공 시에만 로컬 captures 삭제. 기존 두 `Promise.all` 블록에 captures 매핑 추가.
...
- **share_target 처리:** Home mount 시 `parseShareText(location.search)` → 텍스트 있으면 `db.saveCapture({…source:'share'})` 후 `navigate('/', { replace: true })`로 쿼리 제거(중복 적재 방지) + 인박스 새로고침.
