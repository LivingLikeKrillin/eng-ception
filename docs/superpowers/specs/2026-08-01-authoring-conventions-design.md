...
- **R2의 "동사 삼킴" 위반은 기계 검사 대상이 아니다.** V1은 청크 *경계*만 보므로 `[I don't think]`처럼 재배치만 잘못된 경우를 통과시킨다. 굴절 표로 subject 청크를 검사하는 규칙(V7)을 검토했으나 **채택하지 않았다** — 큐레이션 동사 상당수가 명사 동음이의어(`call`·`show`·`look`)라 정상 주어 NP를 오탐하고, 그럼에도 `[I'm afraid]` 같은 보어 삼킴은 못 잡아 사람 판정을 없애지 못한다. **재검토 트리거:** 인벤토리 확장 중 R2 위반이 3건 이상 재발하면, 조동사·부정 토큰으로 좁힌 변형(오탐 ≈0)으로 도입한다. 그때까지는 치트시트의 위반/교정 대조표가 유일한 방어선이다.
