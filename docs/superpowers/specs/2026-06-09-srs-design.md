...
4. `await db.updatePatternSchedule(pid, verb, { ...next, lastGrade: grade, bypassedCount: 0 })` — the store spreads the scheduler's returned `CardSchedule` (which owns `stability/difficulty/state/reps/lapses/nextDueAt/lastReviewedAt`) and only adds `lastGrade` and the avoidance-counter reset (reviewing a card resets its `bypassedCount`).
